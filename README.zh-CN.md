...
3. 然后在启动时通过 `Options::with_fonts(...)`，或运行时通过 `set_font_config(...)` 选择 UI/code 字体族和可选默认字重。`with_ui_families(["MiSans", ...])` 只选择 family；如果想让该 family 以 Medium 字重渲染，需要再配置 `with_ui_weight(FontWeight::MEDIUM)`。
