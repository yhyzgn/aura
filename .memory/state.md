...
## 2026-05-18 Drag backward jump correction

Fixed horizontal-list backward drag jumping ahead of the cursor. Root cause: after live reordering, the previous fix reset the drag pointer anchor to the current cursor position without accounting for the cursor's original grab offset inside the item. DragState now stores the grab offset and computes preview translation from the current slot bounds, so the same point inside the dragged item remains under the cursor even when the live reorder moves the slot backward or forward. Added reusable draggable bounds tracking to support this calculation for HorizontalList and VirtualizedList.

Validation evidence:
- `cargo check -p aura-components -p aura-gallery -p aura-docs --bin check_snippets` passed.
- `cargo test -p aura-components draggable` passed, including backward-slot grab-offset regression coverage.
- `cargo test -p aura-components horizontal_list` passed.
- `cargo test -p aura-components virtualized_list` passed.
- `cargo test -p aura-gallery horizontal_list_demo` passed.
- `cargo test -p aura-gallery virtualized_list_demo` passed.

## 2026-05-18 Drag bounds tracking visibility hotfix

Fixed regression where draggable list items disappeared after adding bounds tracking. Root cause: the bounds tracker wrote back through `Entity::update` during paint for every item, mutating the rendering component while GPUI was painting the tree. Bounds tracking now writes to an internal `Rc<RefCell<Vec<Option<Bounds<Pixels>>>>>` cache without scheduling entity updates or notifications. This keeps item rendering stable while preserving grab-offset-based drag preview positioning.
