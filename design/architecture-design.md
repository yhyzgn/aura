...
Liora 是一套基于 [GPUI](https://github.com/zed-industries/zed)（Zed 编辑器底层 GPU 加速 UI 框架）的企业级 Native 组件库。参考 [Element-Plus](https://element-plus.org/) 的 API 设计规范和组件分类体系，将 Web 端成熟的设计语言映射到 Rust 原生 GUI 领域。
...
| Element-Plus (Vue/Web) | Liora (Rust/GPUI) | 说明 |
|------------------------|------------------|------|
...
├── crates/
│   ├── liora-core/                # 核心 Trait、全局配置、宏、工具函数
│   │   └── src/lib.rs
│   ├── liora-theme/               # 设计系统、Token、亮/暗色模式、色彩计算
│   │   └── src/lib.rs
│   ├── liora-components/          # 全部组件实现
│   │   └── src/
│   │       ├── lib.rs
│   │       ├── button.rs
│   │       ├── input.rs
...
│   │       └── ...
│   ├── liora-tray/                # P11 系统托盘 facade
│   ├── liora-packager/            # P12 打包领域逻辑
│   └── liora-icons/               # 图标库
│       └── src/lib.rs
├── apps/
│   ├── liora-gallery/             # 组件看板（GPUI）
│   │   └── src/
│   │       ├── main.rs
│   │       ├── category.rs
│   │       └── demos/
│   └── liora-docs/                # 官方原生文档主程序（GPUI）
│       ├── content/
...
| Crate | 依赖 | 职责 |
|-------|------|------|
| `liora-theme` | `gpui` | Theme(亮/暗)、Design Tokens、间距/圆角/字号规范、ButtonVariant/ButtonSize/Colors |
| `liora-icons` | `gpui`, `liora-core`, `liora-theme` | Icon 容器（RenderOnce + IntoElement）、IntoIconPath trait |
...
| `xtask` | `liora-packager` | P12 统一打包入口：validate/build/package/ci/smoke/install-smoke |
...
`liora-packager` 与 `xtask package` 共同承担安装器/包产物流程，应用本体仍然是纯 Rust + GPUI native。

- `liora-packager` 负责 app metadata、package format、backend config、SHA-256 checksum、manifest/release-notes 生成。
...
**定位**：liora-gallery 是 Liora 的原生组件看板，功能对标 [Element-Plus 官网](https://element-plus.org/zh-CN/) 的组件总览页。在 GPUI 窗口中按分类层次展示所有已实现组件的交互效果。

**架构**：

```
apps/liora-gallery/src/
...
1. 创建 apps/liora-gallery/src/demos/<name>_demo.rs
   ── 实现 pub fn render(theme: &Theme) -> AnyElement
   ── 内部按小节（Variants/Sizes/States 等）展示组件多种形态

2. 在 apps/liora-gallery/src/demos/mod.rs 注册表添加一行
...
- **即见即得**：`cargo run -p liora-gallery` 直接在原生窗口查看效果
...
**定位**：P10 已交付 Dashboard/监控/报表所需的统计图组件。它们属于 `liora-components`，由 `liora-gallery` 展示，由 `liora-docs` 文档化。
...
2. 当前 Liora 组件模式：`RenderOnce + IntoElement`、全局 Theme、内置唯一 ID、Gallery 自举、Docs 效果→代码。
3. `https://github.com/vicanso/zedis` 仅作为案例参考：其 Metrics 页面使用 GPUI `canvas`，并将图表拆为 scale、axis/grid、shape 层。Liora 可吸收这种分层思想，但必须实现自己的 API、主题与测试。
...
**渲染策略**：在 `canvas` 的 paint 回调中基于实际 `bounds` 计算 scale；使用 `PathBuilder` 生成折线/面积/扇区 path；使用 `paint_quad` 绘制柱体和必要背景；坐标轴文字、legend、tooltip 文案优先复用 Liora Typography 或 GPUI TextSystem。

### 2.5 P8 Liora Docs 主程序架构

**定位**：P8 将 `liora-gallery` 继续作为组件看板，`liora-docs` 作为官方原生文档主程序。官方文档不再作为 Web 文档站交付，而是在 GPUI 原生窗口中完成导航、Markdown 渲染、组件 API 展示和 Live Demo 注入。

**绝对边界**：

- 100% GPUI 原生视窗运行。
- 不建设 VitePress/Web 文档站；`apps/liora-docs` 是 GPUI 原生主程序。
- 不引入浏览器运行时、跨端转译运行时或网页排版模型。
- 文档渲染输出必须是 Rust 数据结构驱动的 Liora/GPUI 原生元素树。

**核心挑战：富文本折行（Word Wrap）**

P8 采用自举策略：`pulldown-cmark` 只负责 Markdown AST/Event 解析；排版、折行、颜色、粗细、行内代码、块级间距等全部交给 Liora Typography/Layout 组件。富文本段落必须能承载多个不同样式的文本片段，并在同一段落容器中流式拼接、自动换行且不截断。
...
- 使用 `Vec` 作为块级容器栈。
- `Start(Tag::Heading | Tag::Paragraph | Tag::List | ...)`：压入 Liora 块级容器。
...
- 每个组件文档独立为 `apps/liora-docs/content/pages/<component>.md`，非组件页面使用 snake_case，例如 `quick_start.md`。
- 代码片段独立放在 `apps/liora-docs/content/snippets/<page>/<case>.rs`。
...
| Element-Plus | Liora 组件 | 阶段 | 说明 |
|-------------|----------|------|------|
| Config Provider | `Config` + `init_liora()` | P0 ✅ | Global 注入，已完成 |
...
Table 是企业级组件库中最复杂、工作量最大的组件。Liora Table 需规划以下能力矩阵：
...
| 任务 | 说明 |
|------|------|
| **Liora Docs 官方文档主程序** | 在 GPUI 原生窗口中承载文档导航、Markdown 内容、组件 Demo 和组件 API 说明 |
...
目标：补齐 Liora 在企业 Dashboard、监控、报表场景中的原生可视化能力。
...
| Hover/Tooltip | ✅ | Line/Area/Bar/Pie/Ring 原生命中测试与 Liora Tooltip portal |
| 性能优化 | ✅ | Line/Area/Sparkline min/max bucket 降采样、稀疏 axis/value labels、避免全量中间 Vec |

验收：所有图表组件已进入 `liora-components` 导出；Gallery 有自举 Demo；Docs 有按“效果 → 代码”组织的页面和完整 `.rs` snippets；相关 scale/shape/builder/hit-test 测试通过。
...
**架构**：
- `liora-icons`：`Icon` 容器（`RenderOnce` + `IntoElement`），`IntoIconPath` trait
- `liora-icons-lucide`：`build.rs` 扫描 `assets/svgs/*.svg` → 生成 `IconName` 枚举
...
| P11 Native Tray | — | ✅ 已完成 | liora-tray facade + Gallery/Docs tray control examples |
| P12 Native Packaging | — | 🔶 Readiness | liora-packager + xtask + CI；签名/真实安装需外部策略 |
...
|------|------|
| `Cargo.toml` | Workspace 根配置 |
| `crates/liora-core/src/lib.rs` | Config, Global, ContextExt, init_liora, Z-Index |
| `crates/liora-theme/src/lib.rs` | Theme, Design Tokens, ButtonVariant/Size |
