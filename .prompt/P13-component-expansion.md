...
- 新增控件：`crates/aura-components/src/<component>.rs`，并在 `lib.rs` 导出。
- 已有控件增强：直接修改现有文件，例如 `tag.rs`、`progress.rs`、`line_chart.rs`、`bar_chart.rs`、`button.rs`、`radio*.rs`、`checkbox*.rs`。
- 新增控件：创建 `apps/aura-gallery/src/demos/<component>_demo.rs` 并注册。
- 已有控件增强：补充现有 `<component>_demo.rs`，不新增平行 demo。
- 新增控件：创建 `apps/aura-docs/content/pages/<component>.md` 与 snippets。
