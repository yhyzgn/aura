...
1. **新增控件**：二维码、代码编辑器、信号图、热力图、独立柱状图、分段比例条、水平可拖动列表、计时器、Label、Operation 等。
2. **既有控件增强**：RingChart、LineChart、BarChart、RingProgress、Button、Tag、Radio、Checkbox、Vertical/List 类控件的高自定义能力。
...
目标：先交付可快速复用、风险低的控件，建立新增组件文档/Demo 基线。
...
- 所有控件导出 API、Gallery demo、Docs 页面、snippet。
- 样式参数覆盖颜色、尺寸、间距、label pattern。
- `cargo check -p aura-components -p aura-gallery -p aura-docs` 通过。
...
- `crates/aura-components/src/<component>.rs`
- `crates/aura-components/src/lib.rs` 导出
- `apps/aura-gallery/src/demos/<component>_demo.rs`
- `apps/aura-gallery/src/demos/mod.rs` 注册，统计图类与 meter 类放在后部数据可视化区域
- `apps/aura-docs/content/pages/<component>.md`
- `apps/aura-docs/content/snippets/<component>/*.rs`

Docs 页面继续保持：**一种效果 → 对应代码 → 下一种效果 → 对应代码**。
...
- [ ] Wave 1 完成并通过 check/test。
- [ ] Wave 2 完成并通过 check/test。
- [ ] Wave 3 完成并通过 check/test。
- [ ] Wave 4 完成并通过 check/test。
- [ ] Wave 5 完成并通过 check/test。
- [ ] Gallery demos 完整、布局不拥挤。
- [ ] Docs 每个控件/增强项均有独立页面或补充章节。
- [ ] `.memory/inventory.md`、`.memory/state.md`、`prompt.md` 更新。
- [ ] 如新增依赖，`Cargo.lock`、许可证说明、依赖取舍记录完成。
