...
将 Aura Gallery 保持为“组件看板”，并把 Aura UI 的官方文档独立为 `aura-docs` 主程序。

P8 不再建设 VitePress/Web 文档站。新的技术路线是：所有文档、示例、导航、Markdown 渲染和活体组件 Demo 都运行在 **GPUI 原生视窗** 内。

## 角色与上下文

你是一位 Rust 与 GPUI 原生框架架构师，负责从 0 到 1 建设 Aura Docs。Aura Docs 是 Aura UI（基于 GPUI 的原生企业级组件库）的官方文档主程序，Aura Gallery 则继续承担组件看板职责。
...
| 文档内容 | `apps/aura-docs/content/pages/*.md` | 每个文档/控件一份 Markdown |
| 示例代码 | `apps/aura-docs/content/snippets/<page>/*.rs` | 代码与 Markdown 分离，按文件命名约定关联 |
...
- [x] 新建 `apps/aura-docs/src/markdown.rs`。
...
- [x] 识别到该语法时，不渲染为普通文字。
- [x] 根据 `component` 实例化真实 Aura 组件，例如 `Button::new("Button").primary()`。
- [x] 将真实 GPUI/Aura view node 直接插入文档流。
...
- [x] 不再搭建 VitePress/Web 文档站；`apps/aura-docs` 保持 GPUI 原生主程序。
- [x] 不再把官方文档作为网页产物维护。
