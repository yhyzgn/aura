...
- [ ] 在 `aura-gallery` 中引入 `pulldown-cmark`。
- [ ] 新建 `apps/aura-gallery/src/markdown.rs`。
- [ ] 实现：`render_markdown(md_text: &str) -> gpui::AnyElement`。
- [ ] 使用 `Vec` 作为栈管理层级：
...
- [ ] 添加 Markdown renderer 回归测试：标题、段落、粗体、斜体、列表、嵌套块级结构。
