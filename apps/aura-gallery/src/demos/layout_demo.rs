use aura_components::{Divider, Space, Text, Title, Row, Col};
...
impl RenderOnce for LayoutDemo {
    fn render(self, _window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        page = page.child(div().flex().flex_row().h(px(60.0)).gap_4()
            .child(Text::new("Left")).child(Divider::new().vertical()).child(Text::new("Right")));

        page = page.child(Title::new("Space 间距").h2());
        page = page.child(Text::new("Horizontal gap 16px:"));
        page = page.child(div().flex().flex_row().items_center()
