use aura_components::{Result, ResultStatus, Button, ButtonVariant, Space};
...
impl Render for ResultDemo {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        div().flex().flex_col().gap_8().p_4().id("result-scroll").overflow_y_scroll()
            .child(
                div().flex().flex_col().gap_2()
                    .child(div().text_lg().font_weight(gpui::FontWeight::BOLD).child("Result 结果页"))
                    .child(div().text_sm().text_color(theme.neutral.text_3).child("用于在任务完成后通知用户结果。"))
            )
            .child(
                ...
                div().flex().flex_col().gap_4()
                    ...
                    .child(
                        Result::new("您的账户存在安全风险")
                            ...
                            .extra(|_, _| Button::new("立即处理").variant(ButtonVariant::Primary).into_any_element())
                    )
            )
            .child(
                div().flex().flex_col().gap_4()
                    ...
                    .child(
                        Result::new("提交失败")
                            ...
                            .extra(|_, _| Button::new("重新提交").variant(ButtonVariant::Primary).into_any_element())
                    )
            )
            .child(
                div().flex().flex_col().gap_4()
