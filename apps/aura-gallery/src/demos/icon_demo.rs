...
use gpui::{div, prelude::*, px, AnyElement, App, Component, Hsla, IntoElement, RenderOnce, Window};
...
impl RenderOnce for IconDemo {
    fn render(self, _window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        page = page.child(
            div().flex().flex_row().gap_4().flex_wrap().items_center()
                .children(icons.iter().map(|(icon, name)| {
                    div().flex().flex_col().gap_1().items_center()
                        .child(build_icon(theme, *icon, 24.0, theme.neutral.icon))
                        .child(div().text_size(px(10.0)).text_color(theme.neutral.text_3).child(*name))
                }))
        );
        ...
        page = page.child(
            div().flex().flex_row().gap_4().items_end()
                .child(icon_labeled_color(theme, IconName::Star, theme.primary.base, "Primary"))
                .child(icon_labeled_color(theme, IconName::Star, theme.info.base, "Info"))
                .child(icon_labeled_color(theme, IconName::Star, theme.success.base, "Success"))
                .child(icon_labeled_color(theme, IconName::Star, theme.warning.base, "Warning"))
                .child(icon_labeled_color(theme, IconName::Star, theme.danger.base, "Danger"))
        );
        page
    }
}
...
fn icon_labeled(theme: &AuraTheme, icon: IconName, sz: f32, label: &str) -> impl IntoElement {
    div().flex().flex_col().gap_1().items_center()
        ...
        .child(div().text_size(px(10.0)).text_color(theme.neutral.text_3).child(label.to_string()))
}

fn icon_labeled_color(theme: &AuraTheme, icon: IconName, color: Hsla, label: &str) -> impl IntoElement {
    div().flex().flex_col().gap_1().items_center()
