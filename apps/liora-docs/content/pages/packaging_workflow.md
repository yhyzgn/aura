...
- crates.io GPUI SDK — `liora` is the one-stop facade published to crates.io; `liora-theme`, `liora-core`, `liora-icons`, `liora-icons-lucide`, `liora-icons-antd`, `liora-icons-ionic`, `liora-icons-tabler`, `liora-icons-carbon`, `liora-icons-material`, `liora-components`, `liora-tray`, `liora-packager`, and `liora-updater` are published with Cargo's `gpui = 0.2.2` registry fallback and are verified through a downstream `[patch.crates-io]` override to the official Zed GPUI git revision.
