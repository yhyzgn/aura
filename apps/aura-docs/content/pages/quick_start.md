...
本页从一个空目录开始，演示如何创建 Rust 项目、接入 GPUI、接入 Aura，并打开一个包含 Aura 组件的原生窗口。示例以 Linux/Fedora 为主，同时列出 macOS、Ubuntu/Debian 和 Windows 的准备事项。
...
Aura 在这些概念之上提供组件、主题、Portal、全局消息层和常用布局组件。
...
GPUI 是原生 GPU UI 框架，Linux 下需要字体、窗口系统、XKB/X11/Wayland、Vulkan 和 C/C++ 构建工具。Aura 仓库已提供 Fedora 脚本：
...
Aura 当前使用 workspace 里的 GPUI git 依赖。外部项目可以先按下面方式配置：
...
2. 调用 `init_aura(cx, Theme::light())` 注册主题。
...
- **弹层位置不对**：弹层类组件通常要捕获 trigger bounds，并通过 Aura Portal 在窗口 overlay 层渲染。
