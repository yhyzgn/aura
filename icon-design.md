# Aura UI - 图标系统架构与代码生成规范 (Icon System Specification)
...
为了提供丝滑的内置图标开发体验，我们需要将 **Lucide** 图标库剥离为一个独立的 Crate（例如命名为 `aura-icons-lucide`）。
...
请在 `aura-icons-lucide` Crate 中编写一个 `build.rs` 脚本。
