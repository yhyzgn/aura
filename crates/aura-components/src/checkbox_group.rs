use gpui::{prelude::*, App, Render, Window, Context, Focusable, FocusHandle, SharedString, Entity};
...
impl CheckboxGroup {
    pub fn new(options: Vec<impl Into<SharedString>>, selected: Vec<usize>, cx: &mut Context<Self>) -> Self {
        ...
        for (i, label) in options.iter().enumerate() {
            ...
            let checkbox = cx.new(|cx| {
                Checkbox::new(is_checked, cx)
                    .label(label.clone())
            });
            
            // Subscribe to each checkbox's change
            cx.subscribe(&checkbox, move |this, _checkbox, event: &CheckboxChanged, cx| {
    ...
    pub fn disabled(mut self, d: bool, cx: &mut Context<Self>) -> Self {
        ...
        for cb in &self.checkboxes {
            cb.update(cx, |cb, cx| { cb.set_disabled(d, cx); });
        }
        self
    }
