...
impl Container {
    pub fn new() -> Self {
        Self { header: None, aside: None, aside_right: false, footer: None, main: vec![] }
    }

    pub fn header(mut self, el: impl IntoElement) -> Self {
        self.header = Some(el.into_any_element()); self
    }
    pub fn aside(mut self, el: impl IntoElement) -> Self {
        self.aside = Some(el.into_any_element()); self
    }
    pub fn aside_right(mut self) -> Self { self.aside_right = true; self }
    pub fn footer(mut self, el: impl IntoElement) -> Self {
        self.footer = Some(el.into_any_element()); self
    }
    pub fn child(mut self, el: impl IntoElement) -> Self {
        self.main.push(el.into_any_element()); self
    }
}
...
impl RenderOnce for Container {
    fn render(self, _window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        let mut page = gpui::div().flex().flex_col().size_full().bg(theme.neutral.body);

        // Header
        if let Some(h) = self.header {
            page = page.child(
                gpui::div().flex_none().h(px(header_h)).w_full()
                    .border_b_1().border_color(theme.neutral.border)
                    .px(px(16.0)).flex().items_center()
                    .child(h)
            );
        }
        ...
        if let Some(f) = self.footer {
            page = page.child(
                gpui::div().flex_none().h(px(footer_h)).w_full()
                    .border_t_1().border_color(theme.neutral.border)
                    .px(px(16.0)).flex().items_center()
                    .child(f)
            );
        }
