...
impl RenderOnce for PageHeader {
    fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        div()
            .flex().flex_col().w_full().p_4().gap_4()
                ...
                div().flex().flex_row().items_center().justify_between()
                    .child(
                        div().flex().flex_row().items_center().gap_3()
                            // Back Button
                            .when_some(self.back_icon, |s, icon| s.child(
                                div()
                                    .id("back-btn")
                                    .cursor_pointer()
                                    .flex().items_center().justify_center()
                                    .w_8().h_8().rounded_full()
                                    ...
                                    .child(Icon::new(icon).size(px(20.0)).color(theme.neutral.text_1))
                            ))
                            // Title
                            .child(div().text_lg().font_weight(gpui::FontWeight::BOLD).text_color(theme.neutral.text_1).child(self.title))
                            // Divider
                            .when(self.sub_title.is_some(), |s| s.child(
                                div().w(px(1.0)).h(px(16.0)).bg(theme.neutral.border)
                            ))
                            // Subtitle
                            .when_some(self.sub_title, |s, sub| s.child(
                                div().text_sm().text_color(theme.neutral.text_3).child(sub)
                            ))
                    )
                    // Extra
                    .when_some(self.extra, |s, extra| s.child(
                        div().flex().flex_row().items_center().child((extra)(window, cx))
                    ))
            )
            // Content
            .when_some(self.content, |s, content| s.child(
                div().child((content)(window, cx))
            ))
            // Footer
            .when_some(self.footer, |s, footer| s.child(
                div().border_t_1().border_color(theme.neutral.border).pt_4()
