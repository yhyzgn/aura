...
impl RenderOnce for Result {
    fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        div()
            .flex().flex_col().items_center().justify_center().w_full().p_12().gap_4()
            .child(
                div().flex().items_center().justify_center().w(px(72.0)).h(px(72.0))
                    .child(match self.icon {
                        Some(icon) => icon,
                        None => Icon::new(icon_name).size(px(72.0)).color(color).into_any_element(),
                    })
            )
            .child(
                div().text_xl().font_weight(gpui::FontWeight::BOLD).text_color(theme.neutral.text_1).child(self.title)
            )
            ...
            .when_some(self.extra, |s, extra| s.child(
                div().mt_4().child((extra)(window, cx))
            ))
    }
}
