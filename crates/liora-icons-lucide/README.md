...
Bundled Lucide SVG icon paths for Liora.

## What it provides

This crate exposes a typed `IconName` enum for the Lucide icon pack. Icons render through `liora-icons::Icon` and GPUI's asset system. Application build scripts can use `liora-icons-optimizer` to package only the icons actually used.
...
- Depend on `liora-icons-lucide` directly when you need fine-grained dependency control or are building another Liora crate.
